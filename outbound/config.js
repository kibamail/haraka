...
exports.set_temp_fail_intervals = function () {
    ...
    function error(i, msg) {
        logger.error(
            exports,
            `temp_fail_intervals syntax error parsing element ${i}: ${msg}`,
        );
        logger.warn(
            exports,
            "Setting outbound temp_fail_intervals to old defaults",
        );
        set_old_defaults();
    }
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (!Number.isInteger(delay[1]))
            return error(i, "multiplier is not an integer");
        ...
        if (num < 5)
            return error(i, "delay time too small, should be >=5 seconds");

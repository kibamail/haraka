...
class HMailItem extends events.EventEmitter {
    //----> $1: TODO: RECEIVE A PARTS OBJECT RATHER THAN A FILENAME
    // PARTS OBJECT PROVIDES FAILURES INFORMATION
    //----> $2: TODO: INSTEAD OF STREAMING THE TODO FROM THE FILE,
    // PASS IN THE TODO INTO THIS CLASS AS AN ARGUMENT.
    constructor(todo) {
        super();
        ...
        this.notes = todo.notes || new Notes();
        this.refcount = 1;
        this.num_failures = todo.num_failures || 0;
        this.todo = todo;
    ...
    data_stream() {
        // return this.todo.message_stream;
        return this.todo.message_stream;
    }
    ...
    send_email_delayed_respond(retval) {
    ...
    async try_deliver() {
        ...
        this.loginfo(`Attempting delivery for mx: ${mx.exchange}`);
    ...
    try_deliver_host_on_socket(mx, host, port, socket) {
        ...
        const send_command = (socket.send_command = (cmd, data) => {
            ...
            if (
                self.force_tls &&
                !["EHLO", "LHLO", "STARTTLS"].includes(cmd.toUpperCase()) &&
                !socket.isSecure()
            ) {
        ...
        function get_reverse_path_with_params() {
            ...
            let rp_params = "";
            if (smtp_properties.smtp_utf8 && has_non_ascii(rp))
                rp_params += " SMTPUTF8";
    ...
    bounce_respond(retval, msg) {
        this.loginfo(`Bounce response from plugins: ${retval} ${msg}`);
    }
    ...
    delivered(
        ...
        this.lognotice({
            "delivered email": this.todo.uuid,

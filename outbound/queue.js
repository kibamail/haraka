...
exports.get_stats = () =>
    `${in_progress}/${exports.delivery_queue.length()}/${exports.temp_fail_queue.length()}`;
...
exports.load_queue = (pid) => {
    ...
    exports._load_cur_queue(pid, exports._add_file, () => {
        logger.info(
            exports,
            `[pid: ${pid}] ${delivery_queue.length()} files in my delivery queue`,
        );
        logger.info(
            exports,
            `[pid: ${pid}] ${load_queue.length()} files in my load queue`,
        );
        logger.info(
            exports,
            `[pid: ${pid}] ${temp_fail_queue.length()} files in my temp fail queue`,
        );
    });
};
...
exports._load_cur_queue = (pid, iteratee, cb) => {
    ...
    fs.readdir(queue_dir, (err, files) => {
        if (err) {
            return logger.error(
                exports,
                `Failed to load queue directory (${queue_dir}): ${err}`,
            );
        }
...
exports.rename_to_actual_pid = (file, parts, cb) => {
    ...
    fs.rename(
        ...
        path.join(queue_dir, new_filename),
        (err) => {
            if (err) {
                return cb(
                    `Unable to rename queue file: ${file} to ${new_filename} : ${err}`,
                );
            }
...
exports._add_file = (file, cb) => {
    ...
    if (parts.next_attempt <= self.cur_time) {
        ...
        logger.debug(
            exports,
            `File ${file} needs processing later: ${parts.next_attempt - self.cur_time}ms`,
        );
    ...
    async.map(
        ...
        (err, results) => {
            ...
            if (searchPid)
                logger.info(
                    exports,
                    `[pid: ${pid}] ${stat_renamed} files old PID queue fixed up`,
                );
    ...
    tl_reader.once("data", (buf) => {
        ...
        td_reader.on("data", (str) => {
            ...
            if (Buffer.byteLength(todo) === todo_len) {
                ...
                todo_struct.rcpt_to = todo_struct.rcpt_to.map(
                    (a) => new Address(a),
                );
...
exports.flush_queue = (domain, pid) => {
    if (domain) {
        exports.list_queue((err, qlist) => {
            if (err)
                return logger.error(exports, `Failed to load queue: ${err}`);
...
exports.ensure_queue_dir = () => {
    ...
    try {
        ...
        let gid;
        if (cfg.user)
            uid = child_process.execSync(`id -u ${cfg.user}`).toString().trim();
        if (cfg.group)
            gid = child_process
                .execSync(`id -g ${cfg.group}`)
                .toString()
                .trim();
        if (uid && gid) {
...
exports._add_hmail = (hmail) => {
    if (hmail.next_process < exports.cur_time) {
        ...
        temp_fail_queue.add(
            hmail.filename,
            hmail.next_process - exports.cur_time,
            () => {
                delivery_queue.push(hmail);
            },
        );

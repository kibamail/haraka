...
describe("tls_socket", () => {
    ...
    describe("get_loud_certs_dir", () => {
        ...
        it("loads certs from test/loud/config/tls", async () => {
            this.socket.config = this.socket.config.module_config(
                path.resolve("test", "loud"),
            );
...
-----END CERTIFICATE-----`;
        ...
        it("returns cert and key from EC pem", async () => {
            const fp = await fs.readFile(
                path.join("test", "config", "tls", "ec.pem"),
            );

...
describe("transaction", () => {
    ...
    it("regression: attachment_hooks before set_banner/add_body_filter", (done) => {
        ...
        this.transaction.end_data(() => {
            this.transaction.message_stream.get_data((body) => {
                assert.ok(
                    /banner$/.test(body.toString().trim()),
                    "banner applied",
                );
                done();
            });
    ...
    it("correct output encoding when content in non-utf8 #2176", (done) => {
        ...
        const message = [
            0x50, 0xf8, 0xed, 0x6c, 0x69, 0xb9, 0x20, 0xbe, 0x6c, 0x75, 0xbb,
            0x6f, 0x76, 0xe8, 0x6b, 0xfd, 0x20, 0x6b, 0xf9, 0xf2, 0xfa, 0xec,
            0x6c, 0x20, 0xef, 0xe2, 0x62, 0x65, 0x6c, 0x73, 0x6b, 0xe9, 0x20,
            0xf3, 0x64, 0x79, 0x2e,
        ];
        const payload = [
            Buffer.from(
        ...
        this.transaction.end_data(() => {
            this.transaction.message_stream.get_data(function (body) {
                assert.ok(
                    body.includes(Buffer.from(message)),
                    "message not damaged",
                );
                done();
            });
    ...
    it("bannering with nested mime structure", (done) => {
        ...
        this.transaction.end_data(() => {
            this.transaction.message_stream.get_data((body) => {
                ...
                assert.ok(
                    /This is an html part/.test(body.toString()),
                    "html content comes through in final message",
                );
                assert.ok(
                    /TEXT_BANNER/.test(body.toString()),
                    "text banner comes through in final message",
                );
                assert.ok(
                    /HTML_BANNER/.test(body.toString()),
    ...
    describe("base64_handling", () => {
        ...
        it("varied-base64-fold-lengths-preserve-data", (done) => {
            ...
            const specimen_path = path.join(
                __dirname,
                "mail_specimen",
                "varied-fold-lengths-preserve-data.txt",
            );
    ...
    describe("boundarymarkercorrupt_test", () => {
        ...
        it("fix mime boundary corruption issue", (done) => {
            ...
            self.transaction.add_data(
                "Date: Wed, 1 Jun 2022 16:44:39 +0530 (IST)\r\n",
            );

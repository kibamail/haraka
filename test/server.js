...
describe("server", () => {
    ...
    describe("get_smtp_server", () => {
        ...
        it("gets a net server object", (done) => {
            this.server
                .get_smtp_server(endpoint("0.0.0.0:2501"), 10)
                .then((server) => {
                    if (!server) {
        ...
        it("gets a TLS net server object", (done) => {
            ...
            this.server
                .get_smtp_server(endpoint("0.0.0.0:2502"), 10)
                .then((server) => {
                    if (!server) {
                    ...
                    (error, info) => {
                        if (error) {
                            ...
                            if (error.message === "socket hang up") {
                                ...
                                /alert certificate required/.test(error.message)
                            ) {
                                // node 18
                                assert.ok(
                                    /alert certificate required/.test(
                                        error.message,
                                    ),
                                );
                            } else {

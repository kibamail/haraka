...
describe("auth_base", () => {
    describe("hook_capabilities", () => {
        ...
        it("with TLS, auth is offered", (done) => {
            ...
            this.plugin.hook_capabilities((rc, msg) => {
                ...
                assert.ok(
                    this.connection.capabilities[0] ===
    ...
    describe("check_plain_passwd", () => {
        ...
        it("valid password", (done) => {
            this.plugin.check_plain_passwd(
                this.connection,
                "test",
                "testpass",
                (pass) => {
    ...
    describe("select_auth_method", () => {
        ...
        it("invalid AUTH method, no result", (done) => {
            this.connection.notes.allowed_auth_methods = [
                "PLAIN",
                "LOGIN",
                "CRAM-MD5",
            ];
    ...
    describe("auth_plain", () => {
        ...
        it("params type=with two line login", (done) => {
            this.plugin.auth_plain(
                (rc) => {
                    assert.equal(
                        this.connection.notes.auth_plain_asked_login,
                        true,
                    );
                    assert.equal(rc, OK);
                    done();
        ...
        it("bad auth", (done) => {
            ...
            this.plugin.check_user(
                (code) => {
                    ...
                    assert.equal(
                        this.connection.notes.auth_custom_note,
                        "custom_note",
                    );
                    done();
                },
        ...
        it("legacyok_nomessage", (done) => {
            this.plugin.check_user(
                (code, msg) => {
                    ...
                    assert.deepEqual(this.connection.notes.resp_strings, [
        ...
        it("AUTH LOGIN", (done) => {
            ...
            this.plugin.hook_unrecognized_command(
                (code) => {
                    ...
                    assert.equal(this.connection.relaying, false);
                    assert.equal(
                        this.connection.notes.auth_login_asked_login,
                        true,
                    );
                    ...
                    this.plugin.hook_unrecognized_command(
                        (code) => {
                            assert.equal(code, OK);
                            assert.equal(
                                this.connection.notes.auth_login_userlogin,
                                "test",
                            );
                            assert.equal(this.connection.relaying, false);
                            this.plugin.hook_unrecognized_command(

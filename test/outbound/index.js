...
describe("outbound", () => {
    ...
    it("log_methods added", () => {
        ...
        for (const level of levels) {
            assert.ok(
                HMailItem.prototype[`log${level.toLowerCase()}`],
                `Log method for level: ${level}`,
            );
        }
    });
    ...
    it("set_temp_fail_intervals coverage", () => {
        ...
        config.cfg.temp_fail_intervals =
            "30s, 1m, 5m, 9m, 15m*3, 30m*2, 1h*3, 2h*3, 1d";
        config.set_temp_fail_intervals();
        assert.deepEqual(
            config.cfg.temp_fail_intervals,
            [
                30, 60, 300, 540, 900, 900, 900, 1800, 1800, 3600, 3600, 3600,
                7200, 7200, 7200, 86400,
            ],
        );
    ...
    describe("get_tls_options", () => {
        beforeEach((done) => {
            ...
            this.obtls.test_config(
                tls_socket.config.module_config(testDir),
                this.outbound.config,
            );
        ...
        it("gets TLS properties from tls.ini.outbound", () => {
            ...
            assert.deepEqual(tls_config, {
                ...
                key: fs.readFileSync(
                    path.resolve("test", "config", "outbound_tls_key.pem"),
                ),
                cert: fs.readFileSync(
                    path.resolve("test", "config", "outbound_tls_cert.pem"),
                ),
                dhparam: fs.readFileSync(
                    path.resolve("test", "config", "dhparams.pem"),
                ),

...
exports.parse_x509 = async (string) => {
    ...
    const keyRe = new RegExp(
        "([-]+BEGIN (?:\\w+ )?PRIVATE KEY[-]+[^-]*[-]+END (?:\\w+ )?PRIVATE KEY[-]+)",
        "gm",
    );
    ...
    if (res.chain?.length) {
        ...
        for (let name of Array.from(
            raw.matchAll(/DNS:([^\s,]+)/gm),
            (m) => m[0],
        )) {
...
exports.load_tls_ini = (opts) => {
    ...
    if (!Array.isArray(cfg.main.no_starttls_ports))
        cfg.main.no_starttls_ports = [];
...
exports.load_default_opts = () => {
    ...
    if (typeof cfg.key[0] === "string") {
        ...
        const asArray = cfg.key.map((keyFileName) => {
            ...
            if (!key) {
                log.error(
                    `tls key ${path.join(this.config.root_path, keyFileName)} could not be loaded.`,
                );
            }
            return key;
        });
    ...
    if (typeof cfg.cert[0] === "string") {
        const asArray = cfg.cert.map((certFileName) => {
            ...
            if (!cert) {
                log.error(
                    `tls cert ${path.join(this.config.root_path, certFileName)} could not be loaded.`,
                );
            }
            return cert;
        });
...
function cleanOcspCache() {
    log.debug(
        `Cleaning ocspCache. How many keys? ${Object.keys(ocspCache.cache).length}`,
    );
